...
export function configure(aurelia: Aurelia) {
  aurelia.use
    ...
    .feature("resources")
    .plugin(PLATFORM.moduleName("aurelia-kis-oidc"), () =>
      configureOpenidPlugin(aurelia)
    );
...
function configureOpenidPlugin(aurelia: Aurelia): PluginConfiguration {
  const webSiteUrl = 'https://aureliakisoidc.z16.web.core.windows.net';
  //const webSiteUrl = "http://localhost:5000";
  return {
    userIdClaimSelector: profile =>
      profile != null && profile.emails != null && profile.emails.length > 0
        ? profile.emails[0]
        : profile.name,
    reconnectPrompt: loginFunc =>
      iziToast.show({
        ...
        buttons: [
          [`<button>Sign in</button>`, (instance, toast) => loginFunc(), true]
        ]
      }),
    userManagerSettings: {
      // Azure B2C
      authority:
        "https://kisssb2c.b2clogin.com/tfp/kisssb2c.onmicrosoft.com/b2c_1_aurelia_kis_oidc_sample/v2.0",
      ...
      scope:
        "https://kisssb2c.onmicrosoft.com/aurelia-kis-oidc-sample/user_impersonation openid",
